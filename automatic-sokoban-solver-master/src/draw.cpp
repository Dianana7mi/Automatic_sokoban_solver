...
void draw_picture::draw_pic(vector<vector<char>>& matrix) {
    ...
    std::string buffer;
    buffer.reserve(4096);
    buffer += "w 上一步, s 下一步, 空格键 返回初始状态, q 退出\n";
    for (auto& row : matrix) {
        for (auto& c : row) {
...
draw_picture::draw_picture() {
    ...
    // Define Colorful Symbols (ANSI Escape Codes)
    // Wall: Dark Gray Block
    symbols[WALL]    = "\033[90m\033[47m██\033[0m"; 
    // Target: Red dots
    symbols[FINAL]   = "\033[91m__\033[0m";
    // Blank: Space
    symbols[BLANK]   = "  ";
    // Box: Yellow Brackets
    symbols[BOX]     = "\033[33m[]\033[0m";
    // Box on Target: Green Brackets
    symbols[REDBOX]  = "\033[92m[]\033[0m";
    // Person: Cyan
    symbols[PERSON]  = "\033[96m@@\033[0m";
    // Person on Target: Cyan
    symbols[PERSONF] = "\033[96m@@\033[0m";
}
